...
from algorithms.rp.models.dict_obs_encoder import DictObsEncoder
from algorithms.rp.models.gru import GRU
    ...
    def __init__(
        ...
        if self.use_rnn:
            self.gru = GRU(
                ...
                num_layers=model_config_dict['custom_model_config'].get('num_lstm_layers',1),
            )
            self.encoder_latent_dim = model_config_dict["lstm_cell_size"]

    def forward(self, inputs: NestedDict) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        encoder_out = self.encoder(inputs)
        
        if self.use_rnn:
            encoder_out, h_next = self.gru(encoder_out, inputs[Columns.STATE_IN][ACTOR]['h'])
        else:
            h_next = None
