...
class meta_rl_env(MultiAgentEnv):
    def __init__(self, flat_env, env_config):
        ...
        self.num_agents = self.flat_env.num_agents
        self.players_ids = self.flat_env.players_ids
    ...
    def reward_fn(self) -> Dict[str, float]:
        ...
        for player_id, r_i in self.original_rewards.items():
            avg_cur_r_minus_i = (cur_total_reward - r_i) / (self.num_agents - 1)
            theta = self.theta_map[self.theta_idx[player_id]]
            new_rewards[player_id] = self.w * self.original_rewards[player_id] + (1-self.w) * (r_i * np.cos(theta) + (avg_cur_r_minus_i * np.sin(theta)))
    ...
    def step(self, action_dict: Dict) -> tuple[ObsType, SupportsFloat, bool, dict[str, Any]]:
        obs, original_rewards, terminated, truncated, infos = self.flat_env.step(action_dict)
        ...
        dones = {'__all__': False}
        if self.step_num % self.max_cycles == 0 or terminated['__all__'] or truncated['__all__']:
            for player_id in self.players_ids:
                if player_id not in obs:
                    obs[player_id] = {}
            
            all_agents_total_reward = np.round(sum(self.flat_env.total_rewards.values()), decimals=2)
            for player_id in self.players_ids:
                infos[player_id]['agent_total_reward'] = np.round(self.flat_env.total_rewards[player_id], decimals=2)
                real_theta = np.arctan2(all_agents_total_reward - infos[player_id]['agent_total_reward'], infos[player_id]['agent_total_reward'])
                infos[player_id]['theta_dis'] = np.round(abs((self.theta_map[self.theta_idx[player_id]] - real_theta + np.pi) % (2 * np.pi) - np.pi), decimals=2)
                infos[player_id]['all_agents_total_reward'] = all_agents_total_reward
                infos[player_id]['compute_total_reward'] = np.round(self.compute_total_rewards[player_id], decimals=2)

                self.trial_infos[player_id]['compute_total_reward'].append(infos[player_id]['compute_total_reward'])
                self.trial_infos[player_id]['real_theta'].append(real_theta)
                self.trial_infos[player_id]['agent_total_reward'].append(infos[player_id]['agent_total_reward'])
                self.trial_infos[player_id]['theta_dis'].append(infos[player_id]['theta_dis'])
                self.trial_infos[player_id]['all_agents_total_reward'].append(all_agents_total_reward)
    ...
    def observation_space(self):
        return gym.spaces.Dict({
            **self.flat_env.observation_space,
            ...
            'other_theta': gym.spaces.Tuple([ gym.spaces.Discrete(8) for _ in range(self.num_agents - 1)]),
            'done': gym.spaces.Discrete(2),
        })
    
    @property
    def action_space(self):
        return self.flat_env.action_space
...
class ObsRewriteEnv(gym.core.ObservationWrapper, MultiAgentEnv):
    def __init__(self, env: Dict, env_config: Dict) -> None:
        ...
        # self._two_hot_obs_keys = env_config.get('two_hot_obs_keys', [])
        # self._two_hot_intervals = env_config.get('two_hot_intervals', 10)
    ...
    def observation(self, observation: Dict):
        ...
        for id, obs in observation.items():
            ...
            _combine_values = {key: value for key, value in obs.items() if key in self._combine_keys}
            for key,value in obs.items():
                if key in self._other_keys:
                    single_player_new_obs[key] = self.preprocessors[key].transform(value)

                if key in self._combine_keys:
                    _combine_values[key] = value

            single_player_new_obs["mlp"] = self.preprocessors['mlp'].transform(_combine_values)
