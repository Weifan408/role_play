...
import envs.social_dilemmas.utility_funcs as util
...
HARVEST_FIRE_CD = 4
HARVEST_REMOVE_STEPS = 25


class HarvestAgent(Agent):
    def __init__(self, agent_id, start_pos, start_orientation, full_map, view_len):
        self.view_len = view_len
        self.fire_past_steps = 0
        self.removed = False
        self.has_removed_steps = 0
        self.removed_steps = HARVEST_REMOVE_STEPS
    ...
    def action_map(self, action_number):
        self.fire_past_steps += 1
        """Maps action_number to a desired action in the map"""
        
        if action_number == 7:
            if self.fire_past_steps <= HARVEST_FIRE_CD:
                return "STAY"
    ...
    def reborn(self, new_pos):
        self.removed = False
        self.punished_flag = False
        self.update_agent_pos(new_pos)
        self.freeze_steps = 0
        self.mark_steps = 0
...
class CleanupAgent(Agent):
    def __init__(self, agent_id, start_pos, start_orientation, full_map, view_len):
        ...
        self.removed = False
        self.has_removed_steps = 0
        self.removed_steps = CLEANUP_REMOVE_STEPS
