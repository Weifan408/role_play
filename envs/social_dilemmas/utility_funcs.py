...
def make_video_from_rgb_imgs(
    rgb_arrs, vid_path, video_name="trajectory", fps=5, format="mp4v", resize=None, slice=False
):
    """
    ...
    for i, image in enumerate(rgb_arrs):
        ...
        image = cv2.resize(image, resize, interpolation=cv2.INTER_NEAREST).astype(np.uint8)
        video.write(image)
    if not slice:
