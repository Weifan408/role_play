...
class MeltingPotEnv(MultiAgentEnv):
    ...
    def __init__(self, env: dmlab2d.Environment):
        ...
        self.single_agent_observation_space = self.observation_space[
            self._ordered_agent_ids[0]
        ]
        self.single_agent_action_space = self.action_space[self._ordered_agent_ids[0]]
        super().__init__()
        self.total_rewards = {agent_id: 0 for agent_id in self._agent_ids}

    def reset(self, *args, **kwargs):
        """See base class."""
        timestep = self._env.reset()
        self.total_rewards = {agent_id: 0 for agent_id in self._ordered_agent_ids}
    ...
    def step(self, action_dict):
        ...
        infos = {agent_id: {} for agent_id in self._ordered_agent_ids}
        observations = timestep_to_observations(timestep)
        return observations, rewards, done, done, infos
    ...
    def _convert_spaces_tuple_to_dict(
        ...
        return spaces.Dict(
            {
                agent_id: (
                    remove_unrequired_observations_from_space(input_tuple[i])
