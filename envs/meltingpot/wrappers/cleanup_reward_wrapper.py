...
class CleanupRewardWrapper(gym.core.RewardWrapper, MultiAgentEnv):
    ...
    def __init__(self, env):
        ...
        self.cleanup_count = {agent_id: 0 for agent_id in self._agent_ids}
    ...
    def reward(self, rewards, infos):
        ...
        if infos[self._agent_ids[0]]["PLAYER_ATE_APPLE"]:
            hidden_rewards[self._agent_ids[0]] += self.w0[2]
            self.harvest_count[self._agent_ids[0]] += 1
        if infos[self._agent_ids[1]]["PLAYER_ATE_APPLE"]:
            hidden_rewards[self._agent_ids[1]] += self.w1[2]
            self.harvest_count[self._agent_ids[1]] += 1

        if infos[self._agent_ids[0]]["PLAYER_CLEANED"]:
            hidden_rewards[self._agent_ids[0]] += self.w0[3]
            self.cleanup_count[self._agent_ids[0]] += 1
        if infos[self._agent_ids[1]]["PLAYER_CLEANED"]:
            hidden_rewards[self._agent_ids[1]] += self.w1[3]
            self.cleanup_count[self._agent_ids[1]] += 1

        rewards[self._agent_ids[0]] = (
            self.w0[4] * rewards[self._agent_ids[0]]
            + hidden_rewards[self._agent_ids[0]]
        )
        rewards[self._agent_ids[1]] = (
            self.w1[4] * rewards[self._agent_ids[1]]
