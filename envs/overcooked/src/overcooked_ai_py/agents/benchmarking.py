...
from envs.overcooked.src.overcooked_ai_py.mdp.overcooked_env import OvercookedEnv
...
class AgentEvaluator(object):
    ...
    def from_mdp_params_finite(
        ...
        assert type(num_mdp) == int and num_mdp > 0, "invalid number of mdp: " + str(
            num_mdp
        )
    ...
    def evaluate_random_pair(
        ...
        agent_pair = AgentPair(
            RandomAgent(all_actions=all_actions), RandomAgent(all_actions=all_actions)
        )
        return self.evaluate_agent_pair(
            agent_pair, num_games=num_games, display=display, native_eval=native_eval
        )
    ...
    def evaluate_agent_pair(
        ...
        if native_eval:
            ...
            horizon_env.start_state_fn = (
                self.env.start_state_fn if start_state_fn is None else start_state_fn
            )
            horizon_env.reset()
    ...
    def _check_trajectories_dynamics(trajectories, verbose=True):
        if any(
            env_params["_variable_mdp"] for env_params in trajectories["env_params"]
        ):
            if verbose:
                print(
