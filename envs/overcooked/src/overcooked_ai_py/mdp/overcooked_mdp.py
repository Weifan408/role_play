...
class OvercookedState(object):
    ...
    def from_players_pos_and_or(
        ...
        return cls(
            ...
            put_first_tomato=put_first_tomato,
        )
    ...
    def from_player_positions(
        ...
        return cls.from_players_pos_and_or(
            dummy_pos_and_or, bonus_orders, all_orders, put_first_tomato
        )
...
class OvercookedGridworld(object):
    ...
    def get_standard_start_state(self):
        ...
        assert not any(start_state.put_first_tomato), (
            start_state.put_first_tomato,
            start_state.timestep,
        )
        return start_state
    ...
    def get_state_transition(
        ...
        sparse_reward_by_agent, shaped_reward_by_agent, shaped_info_by_agent = (
            ...
            #         soup.begin_cooking()
                ...
                if player.get_object().name == "dish" and self.soup_ready_at_location(
                    ...
                    if not soup.is_full:
                        ...
                        if self.layout_name == "far_tomato":
                            if obj.name == "tomato":
                                if self.is_potting_optimal(new_state, old_soup, soup):
                                    ...
                                    shaped_reward[player_idx] += 0
                                if (
                                    len(soup.ingredients) >= 2
                                    and self.is_potting_optimal(
                                        new_state, old_soup, soup
                                    )
                                    and all([x == "tomato" for x in soup.ingredients])
                                ):
                        ...
                        # auto cooking
                        if (
                            self.soup_to_be_cooked_at_location(new_state, i_pos)
                            and len(soup.ingredients) == Recipe.MAX_NUM_INGREDIENTS
                        ):
                            soup.begin_cooking()
    ...
    def get_recipe_value(
        self, state, recipe, discounted=False, base_recipe=None, potential_params={}
    ):
        """
    ...
    def exists_unfull_tomato_soup(self, state):
        ...
        for obj in state.objects.values():
            if (
                obj.name == "soup"
                and not obj.is_full
                and all([x == "tomato" for x in obj.ingredients])
        ...
        if len(soup_sizes) > 0 and all([x == 2 for x in soup_sizes]):
            for player_idx, player in enumerate(state.players):
                if pid == player_idx:
                    continue
                useful = useful and (
                    not (player.has_object() and player.get_object().name == "tomato")
                )
        return useful
    ...
    def featurize_state(self, overcooked_state, mlam, num_pots=2, **kwargs):
        ...
        def make_pot_feature(idx, player, pot_idx, pot_loc, pot_states):
            ...
            feat_dict["p{}_closest_pot_{}_num_tomatoes".format(idx, pot_idx)] = [
                num_tomatoes
            ]
        ...
        for i, player_i in enumerate(overcooked_state.players):
            ...
            player_i_dict = {
                k: v for k, v in features_np.items() if k[:2] == "p{}".format(i)
            }
    ...
    def potential_function(self, state, mp, gamma=0.99):
        ...
        opt_recipe, discounted_opt_recipe_value = self.get_optimal_possible_recipe(
            state,
            None,
            discounted=True,
            potential_params=potential_params,
        ...
        for soup in idle_soups:
            ...
            opt_recipe = self.get_optimal_possible_recipe(
                state, curr_recipe, discounted=True, potential_params=potential_params
            )
