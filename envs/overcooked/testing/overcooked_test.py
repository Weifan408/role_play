...
from envs.overcooked.src.overcooked_ai_py.mdp.overcooked_env import (
    OvercookedEnv,
    DEFAULT_ENV_PARAMS,
)
...
from envs.overcooked.src.overcooked_ai_py.mdp.layout_generator import (
    LayoutGenerator,
    ONION_DISPENSER,
    TOMATO_DISPENSER,
    POT,
...
from envs.overcooked.src.overcooked_ai_py.agents.agent import (
    AgentGroup,
    AgentPair,
    GreedyHumanModel,
    FixedPlanAgent,
...
from envs.overcooked.src.overcooked_ai_py.agents.benchmarking import AgentEvaluator
...
from envs.overcooked.src.overcooked_ai_py.static import TESTING_DATA_DIR
...
class TestRecipe(unittest.TestCase):
    ...
    def test_recipes_generation(self):
        ...
        self.assertRaises(
            AssertionError, Recipe.generate_random_recipes, min_size=3, max_size=2
        )
        self.assertRaises(
            AssertionError,
            Recipe.generate_random_recipes,
        ...
        for _ in range(100):
            self.assertCountEqual(
                two_sized_recipes,
                Recipe.generate_random_recipes(
                    n=3, min_size=2, max_size=2, ingredients=["onion", "tomato"]
                ),
            )
...
class TestGridworld(unittest.TestCase):
    ...
    def test_transitions_and_environment(self):
        ...
        expected_path = os.path.join(
            TESTING_DATA_DIR, "test_transitions_and_environments", "expected.json"
        )
    ...
    def test_potential_function(self):
        ...
        self.assertLess(
            val15, val16, "Moving towards soup with dish should increase potential"
        )
        ...
        self.assertLess(
            val17, val18, "Moving towards failed soup should increase potential"
        )
...
class TestFeaturizations(unittest.TestCase):
    ...
    def test_state_featurization(self):
        ...
        for num_pots in range(3):
            featurized_observations = [
                [
                    self.base_mdp.featurize_state(state, self.mlam, num_pots=num_pots)
...
class TestOvercookedEnvironment(unittest.TestCase):
    ...
    def test_display(self):
        ...
        env.get_rollouts(
            self.rnd_agent_pair, 1, display=True, info=False, dir=self.dummy_dir
        )
