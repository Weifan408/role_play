...
from envs.overcooked.src.overcooked_ai_py.agents.agent import RandomAgent
...
from envs.overcooked.src.overcooked_ai_py.mdp.overcooked_mdp import Recipe
...
def test_render_state_from_dict(test_dict):
    ...
    actual_result = pygame.surfarray.array3d(
        StateVisualizer(**test_dict["config"]).render_state(**test_dict["kwargs"])
    )
...
class TestStateVisualizer(unittest.TestCase):
    ...
    def test_setting_up_configs(self):
        ...
        self.assertRaises(
            AssertionError, StateVisualizer.configure_defaults, **invalid_kwargs
        )
    ...
    def test_hud_display(self):
        for d in load_from_json(
            os.path.join(state_visualizer_dir, "render_state_data_test_hud.json")
        ):
            test_render_state_from_dict(d)

    def test_differnet_sizes(self):
        for d in load_from_json(
            os.path.join(state_visualizer_dir, "render_state_data_test_sizes.json")
        ):
            test_render_state_from_dict(d)
    ...
    def test_cooking_timer_display(self):
        for d in load_from_json(
            os.path.join(
                state_visualizer_dir, "render_state_data_test_cooking_display.json"
            )
        ):
            test_render_state_from_dict(d)
    ...
    def test_various_states(self):
        for d in load_from_json(
            os.path.join(state_visualizer_dir, "render_state_data_test_various.json")
        ):
            test_render_state_from_dict(d)
    ...
    def test_generated_layout_states(self):
        for d in load_from_json(
            os.path.join(
                state_visualizer_dir, "render_state_data_test_generated_layout.json"
            )
        ):
            test_render_state_from_dict(d)
    ...
    def test_action_probs_display(self):
        for d in load_from_json(
            os.path.join(
                state_visualizer_dir, "render_state_data_test_action_probs_display.json"
            )
        ):
            test_render_state_from_dict(d)
