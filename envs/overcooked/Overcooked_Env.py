...
import time
import numpy as np
from envs.overcooked.src.overcooked_ai_py.utils import (
    mean_and_std_err,
    append_dictionaries,
)
from envs.overcooked.src.overcooked_ai_py.mdp.actions import Action, Direction
from envs.overcooked.src.overcooked_ai_py.mdp.overcooked_mdp import (
    OvercookedGridworld,
    EVENT_TYPES,
)
from envs.overcooked.src.overcooked_ai_py.mdp.overcooked_trajectory import (
    TIMESTEP_TRAJ_KEYS,
    EPISODE_TRAJ_KEYS,
    DEFAULT_TRAJ_KEYS,
)
from envs.overcooked.src.overcooked_ai_py.planning.planners import (
    MediumLevelActionManager,
    MotionPlanner,
    NO_COUNTERS_PARAMS,
)
from envs.overcooked.src.overcooked_ai_py.visualization.state_visualizer import (
    StateVisualizer,
)
import imageio
import os
...
from envs.overcooked.script_agent import SCRIPT_AGENTS
...
class OvercookedEnv(object):
    ...
    def get_rollouts(
        ...
        from envs.overcooked.src.overcooked_ai_py.agents.benchmarking import (
            AgentEvaluator,
        )
...
class Overcooked(gym.Env):
    ...
    def __init__(
        ...
        if baselines_reproducible:
            ...
            np.random.seed(0)
        self._initial_reward_shaping_factor = env_config[
            "initial_reward_shaping_factor"
        ]
        self._agent_ids = ["agent_0", "agent_1"]
        self.reward_shaping_factor = env_config["reward_shaping_factor"]
        self.reward_shaping_horizon = env_config["reward_shaping_horizon"]
        self.store_traj = getattr(env_config, "store_traj", False)
        self.rank = rank
        self.add_hidden_reward = env_config.get("add_hidden_reward", True)
        self.use_render = env_config["use_render"]
        self.num_agents = env_config["num_agents"]
        self.layout_name = env_config["name"]
        self.episode_length = env_config["max_cycles"]
        self.random_start_prob = getattr(env_config, "random_start_prob", 0.0)
        self.stuck_time = stuck_time
        self.history_sa = []
        self.traj_num = 0
        self.step_count = 0
        self.run_dir = run_dir
        self.w0, self.w1 = None, None
        ...
        self.base_mdp = OvercookedGridworld.from_layout_name(
            env_config["name"], rew_shaping_params=rew_shaping_params
        )
        ...
        if env_config.get("script_agent", None):
            script_agent_name = env_config["script_agent"]
            self.script_agent_id = env_config.get("script_agent_id", 1)
            self.script_agent[self.script_agent_id] = SCRIPT_AGENTS[script_agent_name[7:]]()
            self.script_agent[self.script_agent_id].reset(self.base_env, self.base_env.state, self.script_agent_id)
    ...
    def reset_featurize_type(self, featurize_type=("ppo", "ppo")):
        ...
        self.observation_space = {
            self._agent_ids[i]: {
                "featurize_obs": self._observation_space(
                    featurize_type[i]
                ),  # (5, 5, 20)
                "featurize_shared_obs": self._setup_share_observation_space(),  # (5, 5, 40)
            }
            for i in range(self.num_agents)
        }

        self.action_space = {
            self._agent_ids[i]: gym.spaces.Discrete(len(Action.ALL_ACTIONS))
            for i in range(self.num_agents)
        }
    ...
    def step(self, action):
        ...
        agent_action, other_agent_action = [
            Action.INDEX_TO_ACTION[a] if a is not None else None for a in action
        ]
        ...
        # can_begin_cook_soup
        can_begin_cook_info = [
            self.mdp.can_begin_cook_soup(next_state, agent_id)
        ...
        return self._to_RLlib_API(
            both_agents_ob, share_obs, reward, done, info, available_actions
        )
    ...
    def reset(self, reset_choose=True):
        ...
        return self._to_RLlib_API(
            both_agents_ob, share_obs, None, None, None, available_actions
        )
    ...
    def render(self):
        try:
            ...
            imageio.mimsave(
                save_dir + f'/reward_{self.traj["ep_returns"][0]}.gif',
                imgs,
                duration=0.05,
            )
            imgs_dir = os.listdir(save_dir)
    ...
    def _to_RLlib_API(
        ...
        observations = {
            self._agent_ids[0]: {
                "featurize_obs": observations[0],
                "featurize_shared_obs": share_observations[0],
            },
            self._agent_ids[1]: {
                "featurize_obs": observations[1],
                "featurize_shared_obs": share_observations[1],
            },
        }
        ...
        if infos is None:
            ...
            for key, values in infos.items():
                if isinstance(values, list):
                    if len(values) == 2:
                        new_infos[self._agent_ids[0]][key] = values[0]
                        new_infos[self._agent_ids[1]][key] = values[1]
                    elif len(values) == 1:
                        new_infos[self._agent_ids[0]][key] = values[0]
                        new_infos[self._agent_ids[1]][key] = values[0]
                    else:
                        new_infos[self._agent_ids[0]][key] = values
                        new_infos[self._agent_ids[1]][key] = values
                else:
                    new_infos[self._agent_ids[0]][key] = values
                    new_infos[self._agent_ids[1]][key] = values
            new_infos[self._agent_ids[0]]["available_actions"] = available_actions[0]
            new_infos[self._agent_ids[1]]["available_actions"] = available_actions[1]
            infos = new_infos
        ...
        rewards = {
            self._agent_ids[0]: rewards[0][0],
            self._agent_ids[1]: rewards[1][0],
        }

        dones = {
            self._agent_ids[0]: dones[0],
            self._agent_ids[1]: dones[1],
            "__all__": dones[0],
        }

        return observations, rewards, dones, dones, infos

    def set_w(self, w0, w1):
        self.w0 = w0
        self.w1 = w1
