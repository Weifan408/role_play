...
def compute_valid_map(mdp, state, player_idx, terrain_type, obj_lst):
    ...
    valid_map = np.zeros(
        (len(mdp.terrain_mtx), len(mdp.terrain_mtx[0])), dtype=np.int32
    )
            ...
            for obj in obj_lst:
                if obj == "onion":
                    valid_map[y, x] += (terrain == "O") or (
                        terrain == "X"
                        and state.has_object(pos)
                        and state.get_object(pos).name == "onion"
                    )
                elif obj == "tomato":
                    valid_map[y, x] += (terrain == "T") or (
                        terrain == "X"
                        and state.has_object(pos)
                        and state.get_object(pos).name == "tomato"
                    )
                elif obj == "dish":
                    valid_map[y, x] += (terrain == "D") or (
                        terrain == "X"
                        and state.has_object(pos)
                        and state.get_object(pos).name == "dish"
                    )
                elif obj == "cooking_soup":
                    if (
                        terrain == "P"
                        and state.has_object(pos)
                        and state.get_object(pos).name == "soup"
                    ):
                    ...
                    if terrain == "P":
                        if (
                            ...
                            if (
                                num_t > 0
                                and num_o == 0
                                and len(ingredients) < mdp.num_items_for_soup
                            ):
                                valid_map[y, x] += 1
                    ...
                    if terrain == "X":
                        valid_map[y, x] += (
                            not state.has_object(pos) and player.has_object()
                        )
                    elif terrain == "P":
...
def interact(
    ...
    if goal is None:
        ...
        candidates = [
            ...
            if dist[y, x] != -1 and (move_mask is None or move_mask[y, x] == 1)
        ]
        if len(candidates) == 0:
            candidates = mdp.get_valid_player_positions()
