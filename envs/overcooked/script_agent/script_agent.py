...
import numpy as np
from envs.overcooked.script_agent.base import BaseScriptAgent
from envs.overcooked.script_agent.script_period import SCRIPT_PERIODS_CLASSES
import envs.overcooked.script_agent.utils as utils
from envs.overcooked.src.overcooked_ai_py.mdp.actions import Direction, Action
import random
import functools
    ...
    def step(self, mdp, state, player_idx):
        ...
        if self._current_period_name == "pickup_soup_and_deliver":
            if not (
                ...
                self._current_period.reset(mdp, state, player_idx)
        return super(Place_Onion_and_Deliver_Soup_Agent, self).step(
            mdp, state, player_idx
        )
    ...
    def step(self, mdp, state, player_idx):
        ...
        if self._current_period_name == "pickup_soup_and_deliver":
            if not (
                ...
                self._current_period.reset(mdp, state, player_idx)
        return super(Place_Tomato_and_Deliver_Soup_Agent, self).step(
            mdp, state, player_idx
        )
...
class Noisy_Agent(RandomScriptAgent):
    def __init__(self, onion_ratio, soup_ratio, noise_ratio):
        super().__init__(
            {
                "pickup_onion_and_place_in_pot": dict(
                    prob=onion_ratio * (1.0 - noise_ratio), args=dict()
                ),
                "pickup_onion_and_place_random": dict(
                    prob=onion_ratio * noise_ratio, args=dict()
                ),
                "pickup_soup_and_deliver": dict(
                    prob=soup_ratio * (1.0 - noise_ratio), args=dict()
                ),
                "pickup_soup_and_place_random": dict(
                    prob=soup_ratio * noise_ratio, args=dict()
                ),
            }
        )
...
SCRIPT_AGENTS = {
    "place_onion_in_pot": functools.partial(
        SinglePeriodScriptAgent, period_name="pickup_onion_and_place_in_pot"
    ),
    "place_tomato_in_pot": functools.partial(
        SinglePeriodScriptAgent, period_name="pickup_tomato_and_place_in_pot"
    ),
    "deliver_soup": functools.partial(
        SinglePeriodScriptAgent, period_name="pickup_soup_and_deliver"
    ),
    ...
    # "noisy": Noisy_Agent,
    "put_onion_everywhere": functools.partial(
        SinglePeriodScriptAgent, period_name="put_onion_everywhere"
    ),
    "put_tomato_everywhere": functools.partial(
        SinglePeriodScriptAgent, period_name="put_tomato_everywhere"
    ),
    "put_dish_everywhere": functools.partial(
        SinglePeriodScriptAgent, period_name="put_dish_everywhere"
    ),
    ...
    "mixed_order": functools.partial(
        SinglePeriodScriptAgent, period_name="mixed_order"
    ),
}
