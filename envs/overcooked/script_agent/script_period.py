from envs.overcooked.script_agent.base import BaseScriptPeriod
...
class Pickup_Object(BaseScriptPeriod):
    ...
    def step(self, mdp, state, player_idx):
        ...
        action, self.__random_pos = utils.random_move(
            mdp, state, player_idx, pre_goal=self.__random_pos
        )
        return action
    ...
    def step(self, mdp, state, player_idx):
        ...
        if self.__stage == 1:
            if self.__current_period.done(mdp, state, player_idx):
                assert (
                    player.has_object() and player.get_object().name in self.target_obj
                )
                self.__stage = 2
                self.__current_period = Put_Object(
                    terrain_type="P", random_put=self.random_pot
                )
            else:
        ...
        if self.__stage == 1:
            ...
            if self.__current_period.done(mdp, state, player_idx):
                ...
                if player.get_object().name == "onion":
                    ...
                    raise RuntimeError(
                        f"Unexpected: Player has object {player.get_object().name}"
                    )
                self.__current_period = Put_Object(
                    obj=obj, terrain_type="P", random_put=self.random_pot
                )
            else:
                return self.__current_period.step(mdp, state, player_idx)
...
class Pickup_Ingredient_and_Place_Random(BaseScriptPeriod):
    def __init__(
        self, random_put=True, random_ingredient=True, obj=["onion", "tomato"]
    ):
        """
        random_put: bool
            if True, place the object to random position when the player starts with
    ...
    def step(self, mdp, state, player_idx):
        ...
        if self.__stage == 1:
            if self.__current_period.done(mdp, state, player_idx):
                ...
                self.__stage = 2
                self.__current_period = Put_Object(
                    terrain_type="XOPTDS", random_put=True
                )
            else:
                return self.__current_period.step(mdp, state, player_idx)
        return self.__current_period.step(mdp, state, player_idx)
...
class Put_Onion_Everywhere(Put_Ingredient_Everywhere):
    def __init__(self, random_put=True, random_onion=True):
        super(Put_Onion_Everywhere, self).__init__(
            random_put=random_put, random_ingredient=random_onion, obj="onion"
        )
...
class Pickup_Soup_and_Deliver(BaseScriptPeriod):
    def __init__(self, random_dish=True, random_soup=True):
        ...
        self.__stage = 1
        self.__current_period = Pickup_Soup(
            random_dish=self.random_dish, random_soup=self.random_soup
        )
    ...
    def step(self, mdp, state, player_idx):
        ...
        if self.__stage == 1:
            if self.__current_period.done(mdp, state, player_idx):
                ...
                # this is a quick hack to use put as deliver
                self.__current_period = Put_Object(
                    terrain_type="XOTPDS", random_put=True
                )
            else:
                return self.__current_period.step(mdp, state, player_idx)
        return self.__current_period.step(mdp, state, player_idx)
