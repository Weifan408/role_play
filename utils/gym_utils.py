...
@numba.njit()
def timestep_to_observations_with_numba(timestep: dm_env.TimeStep):
    gym_observations = {}
    for index, observation in enumerate(timestep.observation):
        gym_observations[PLAYER_STR_FORMAT.format(index=index)] = {
            key: value
...
@numba.njit()
def get_custom_rewards(rewards, cur_theta, target_theta, w):
    
    return [
        rewards[index] - w * np.abs(cur_theta[index] - target_theta[index])
        for index in range(len(rewards))
    ]

@numba.njit()
def get_cur_theta(rewards):
    others_rewards = [
        sum(rewards) - rewards[index] for index in range(len(rewards))
    ]

    return [
        np.arctan2(others_rewards[agent_id], rewards[agent_id])
